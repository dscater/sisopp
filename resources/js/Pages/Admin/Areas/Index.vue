...
import { useAreas } from "@/composables/areas/useAreas";
...
const { setArea, limpiarArea } = useAreas();
    ...
    },
    {
        ...
        render: function (data, type, row) {
            ...
            if (
                ...
                props_page.auth?.user.permisos.includes("areas.edit")
            ) {
            ...
            if (
                props_page.auth?.user.permisos == "*" ||
                props_page.auth?.user.permisos.includes("areas.destroy")
            ) {
...
const accionesRow = () => {
    // editar
    $("#table-area").on("click", "button.editar", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("areas.show", id)).then((response) => {
    ...
    // eliminar
    $("#table-area").on("click", "button.eliminar", function (e) {
        e.preventDefault();
...
onMounted(async () => {
    datatable = initDataTable("#table-area", columns, route("areas.api"));
...
<template>
    <Head title="Áreas de producción"></Head>

    <!-- BEGIN breadcrumb -->
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="javascript:;">Inicio</a></li>
        <li class="breadcrumb-item active">Áreas de producción</li>
    </ol>
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Áreas de producción</h1>
    <!-- END page-header -->
